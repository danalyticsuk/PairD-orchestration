...
class Markov(object):
    def __init__(self):
        with open(os.path.join(CURRENT_DIR, 'markov_characters.yaml'), 'r') as yaml_file:
            yaml_data = yaml.safe_load(yaml_file)
            
        self.accepted_chars = yaml_data['markov_characters']
        self.pos = {char: idx for idx, char in enumerate(self.accepted_chars)}
        self.gibberish_model = pickle.load(open(os.path.join(CURRENT_DIR, 'markov_model.pki'), 'rb'))
